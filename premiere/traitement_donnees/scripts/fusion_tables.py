def fusionner(table1, table2):
    ...
    for i in range(...):  # on parcourt la table2 par indice
        for cle, val in table2[i].items():  # pour chaque enregistrement (chaque dictionnaire) de la table2
            if ... in table1[i]:  # si la clé était déjà présente dans la table1
                if val != table1[...][...]:  # si la valeur est différente de celle de la table 1
                    ... = val  # on remplace l'ancienne valeur par la nouvelle
            else:  # si la clé n'était pas présente dans la table1
                ...  # on ajoute la nouvelle clé en l'associant à la valeur val
