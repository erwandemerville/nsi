...
def trouver_version2(liste, element):
...
def dichotomie(liste, element):
    ...
    fin = len(liste) - 1  # Indice de fin de la liste
    while debut <= fin:  # Tant qu'il reste des éléments dans la partie de la liste entre debut et fin
        m = (debut + fin) // 2  # Indice de l'élément du milieu entre debut et fin
        if liste[m] == element:
            # On a trouvé element
            return True
        elif liste[m] < element:  # v se situe après l'élément du milieu
