...
!!! warning "Problème du chiffrement symétrique"
    Le chiffrement symétrique possède un **défaut majeur** :  
    Pour communiquer, il faut dans un premier temps **transmettre la clé de chiffrement** qui sera utilisée par les deux participants.

    Pour cela, deux solutions :

    - par rencontre physique, ce qui n'est pas toujours très pratique,
    - par **transmission non sécurisée** (et donc avec un risque d'interception).

Pour répondre à ce risque lié à la **transmission** d'une **clé de chiffrement symétrique**, des techniques de **chiffrement asymétrique** ont été développées dès les années *1970*.

!!! abstract "Chiffrement asymétrique"
    La **cryptographie asymétrique**, également appelée **cryptographie à clé publique** repose sur un ==**couple de clés**== **différentes** mais ==**liées mathématiquement**== pour garantir la **confidentialité** et l’**authenticité** des communications :

	- une ==**clé publique**==, qui peut être **partagée** avec tout le monde,
	- une ==**clé privée**==, qui doit rester **secrète**.

On présentera 3 méthodes de chiffrement asymétrique :
* avec les **puzzles de Merkle**
* avec la **méthode de Diffie-Hellman**
* avec le **système RSA**
